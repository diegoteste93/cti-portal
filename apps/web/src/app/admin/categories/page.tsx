    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8 overflow-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => setShowForm(!showForm)} className="btn-primary">{showForm ? 'Cancel' : '+ Add Category'}</button>
        </div>
          ...
          <form onSubmit={handleCreate} className="card mb-6 space-y-4">
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input value={form.slug} onChange={(e) => setForm({...form, slug: e.target.value})} className="input-field" required placeholder="e.g. my_category" />
              </div>
              <div>
            ...
            <button type="submit" className="btn-primary" disabled={saving}>{saving ? 'Saving...' : 'Create Category'}</button>
          </form>
        )}
